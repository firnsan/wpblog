...
function wp_get_attachment_url( $post_id = 0 ) {
	...
	if ( $file = get_post_meta( $post->ID, '_wp_attached_file', true) ) {
		...
		if ( ($uploads = wp_upload_dir()) && false === $uploads['error'] ) {
			...
			// $basedir is a linux path you config as DocumentRoot
			$basedir = substr($uploads['basedir'], strlen($_SERVER[DOCUMENT_ROOT]));
			...
			if ( 0 === strpos( $file, $uploads['basedir'] ) ) {
				...
				$url = trailingslashit( $basedir . '/' . _wp_get_attachment_relative_path($file)) . basename( $file );
			} else {
